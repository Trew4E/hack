...
/**
 * MagicBento — renders children inside a bento grid with spotlight + particle effects.
 * Pass cards as children; each child should be a <BentoCard> component.
 * If no children are provided, renders default demo cards.
 */
const MagicBento = ({
    children,
    textAutoHide = true,
    ...
    const renderCard = (child, index) => {
        const cardProps = {
            ...
            style: { backgroundColor: 'rgba(6, 0, 16, 0.35)', '--glow-color': glowColor }
        };
        ...
        if (enableStars) {
            return (
                ...
                >
                    <div className="bento-card-inner">{child}</div>
                </ParticleCard>
            );
        }
    ...
    return (
        <>
            ...
            <div className={`card-grid bento-section${autoLayout ? ' card-grid--auto' : ''}`} ref={gridRef}>
                {wrappedChildren.map((child, i) => renderCard(child, i))}
            </div>
        </>
    );
};
